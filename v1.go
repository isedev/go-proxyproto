...
func parseVersion1(reader *bufio.Reader) (*Header, error) {
	...
	// until the delimiter is found; an abusive client could easiily disrupt a
	// server by sending a large amount of data that do not contain CRLF or LF
	// delimiter. Another means of attack would be to start connectons and simply
	// not send data after the initial PROXY signature bytes, accumulating a large
	...
	// reading beyond the end of the header. However, we need one trick to harden
	// against partial header attacks (block on read) - per spec:
	//
